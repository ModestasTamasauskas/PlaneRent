...
appMainModule.controller("AccountLoginViewModel", function ($scope, $http, $location, viewModelHelper) {
    ...
    $scope.accountModel = new PlaneRental.AccountLoginModel();
    $scope.returnUrl = '';
            ...
            viewModelHelper.apiPost('api/account/login', $scope.accountModel,
                function (result) {
                    if ($scope.returnUrl != '' && $scope.returnUrl.length > 1) {
                        window.location.href = PlaneRental.rootPath + $scope.returnUrl.substring(1);
                    }
                    else {
                        window.location.href = PlaneRental.rootPath;
                    }
                });
        }
        else
